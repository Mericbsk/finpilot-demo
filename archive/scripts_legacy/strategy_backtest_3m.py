...
try:
    ...
    for symbol in SYMBOLS:
        try:
            df_sym = raw_data.copy() if len(SYMBOLS) == 1 else raw_data[symbol].copy()
...
for symbol, df_ind in indicator_cache.items():
    ...
    for _i, date in enumerate(test_dates):
        ...
        try:
            ...
            if core_signal:
                entry_ok = bool(score == 3 or score == 4 or score == 2 and mtf_ok)
            else:
                entry_ok = False
...
entry_signals = df_signals[df_signals["entry_ok"]]
no_entry = df_signals[not df_signals["entry_ok"]]

# Regime ve Direction dağılımı
regime_true = (df_signals["regime"]).sum()
direction_true = (df_signals["direction"]).sum()
        ...
        for symbol in list(sim.open_positions.keys()):
            if symbol in indicator_cache:
                ...
                if date_ts in df_sym.index:
                    ...
                    c = float(row["Close"])
                    sim.check_exits(symbol, h, low, c, date)
