...
def run_wfo_grid_search(
    rsi_upper_list=None,
    volume_mult_list=None,
    momentum_pct_list=None,
    start_date="2023-01-01",
    end_date="2023-12-31",
    train_window_days=120,
    test_window_days=30,
):
    if momentum_pct_list is None:
        momentum_pct_list = [0.5, 1.0, 1.5, 2.0]
    if volume_mult_list is None:
        volume_mult_list = [1.0, 1.1, 1.2, 1.3]
    if rsi_upper_list is None:
    ...
    while i + train_window_days + test_window_days <= len(date_range):
        ...
        for rsi_upper, vol_mult, mom_pct in itertools.product(
            rsi_upper_list, volume_mult_list, momentum_pct_list
        ):
            import scanner
            ...
            bt = SimpleBacktest()
            bt.backtest_period(
                symbols, train_start.strftime("%Y-%m-%d"), train_end.strftime("%Y-%m-%d")
            )
        ...
        if best_params is not None:
            ...
            bt_test.backtest_period(
                symbols, test_start.strftime("%Y-%m-%d"), test_end.strftime("%Y-%m-%d")
            )
...
def run_grid_search(
    ...
    df_results = df_results.sort_values(
        by=["CAGR", "Sharpe", "WinRate"], ascending=[False, False, False]
    )
