...
def calculate_weighted_sentiment(frame: pd.DataFrame, config: WeightedSentimentConfig = WeightedSentimentConfig()) -> pd.Series:
    ...
    if config.weight_col and config.weight_col in df.columns:
        ...
        weighted_values = (sentiment * weights).ewm(alpha=config.decay, adjust=False, min_periods=config.min_periods).mean()
        normaliser = weights.ewm(alpha=config.decay, adjust=False, min_periods=config.min_periods).mean()
        result = weighted_values / normaliser.replace(to_replace=0.0, value=np.nan)
    else:
        result = sentiment.ewm(alpha=config.decay, adjust=False, min_periods=config.min_periods).mean()
