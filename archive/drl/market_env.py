...
class MarketEnv(BaseEnv):
    ...
    def __init__(
        ...
        self._timestamps = (
            episode.timestamps.tolist() if episode.timestamps is not None else None
        )
    ...
    def step(self, action):  # type: ignore[override]
        ...
        commission_rate = (self._cost_model.commission_bps + self._cost_model.slippage_bps) / 10000.0
    ...
    def _build_info(self, pnl: float, drawdown: float, reward: float) -> Dict[str, Union[float, str]]:
        timestamp = None
