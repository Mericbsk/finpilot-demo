...
class WalkForwardTrainer:
    ...
    def _create_model(self, pipeline: FeaturePipeline, split: EpisodeData):
        ...
        if algo == "PPO":
            hyperparams.update(dict(gae_lambda=self.algo_config.gae_lambda, ent_coef=self.algo_config.ent_coef, vf_coef=self.algo_config.vf_coef))
    ...
    def _compute_metrics(self, history: List[Dict[str, Union[float, str]]]) -> EvaluationMetrics:
        ...
        rewards = np.array([
            float(entry.get("reward", 0.0)) if isinstance(entry.get("reward"), (int, float)) else 0.0
            for entry in history
        ])
        equity = np.array([
            float(entry.get("equity", 1.0)) if isinstance(entry.get("equity"), (int, float)) else 1.0
            for entry in history
        ])
