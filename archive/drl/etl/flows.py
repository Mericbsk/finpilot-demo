...
_PREFECT_TASK, _PREFECT_FLOW, _PREFECT_GET_RUN_LOGGER, _PREFECT_FLOW_RUN = _load_prefect_components()
PREFECT_AVAILABLE = _PREFECT_TASK is not None


def _missing_prefect(*_args: Any, **_kwargs: Any) -> None:  # pragma: no cover
    raise RuntimeError("Prefect>=2 is required to use the ETL flow. Install it via requirements-etl.txt.")
