...
def _summarize_sentiment(history: pd.DataFrame, column: str, window: int) -> AlternativeSignal:
    ...
    if abs_pct < 1.0:
        description = "Son 4 saatte duygu skoru anlamlı bir değişim göstermedi; piyasa hissiyatı dengede."
        strength = "neutral"
    else:
        rising = delta >= 0.0
        ...
        description = (
            f"Son 4 saatte duygu skoru %{abs_pct:.1f} {speed}{verb}; bu kısa vadede {impact} gösteriyor."
        )
        if abs_pct >= 10.0:
...
def _summarize_whale_flow(history: pd.DataFrame, column: str, window: int) -> AlternativeSignal:
    ...
    if magnitude < 1.0:
        ...
        qualifier = "normalin 2 katından fazla" if magnitude >= 2.0 else f"normalin yaklaşık {magnitude:.1f} katı"
        implication = "kurumsal talebin güçlendiğini" if z_score > 0 else "kurumsal satış baskısının arttığını"
...
def _determine_exit_level(
    ...
    limits = [value for value in (regime_drawdown, user_drawdown) if value is not None and value > 0.0]
...
def _compose_risk_paragraph(whale_sentence: str, exit_level: Optional[_ExitLevel]) -> str:
    ...
    if exit_level is None:
        ...
        exit_sentence = (
            f"Fiyat %{drop_pct:.1f} gerilerse (yaklaşık {exit_level.price:,.2f}) pozisyonu kapatmayı düşünün."
        )
...
def estimate_regime_success(
    ...
    rewards_array = np.asarray([r for r, reg in zip(rewards, regimes) if reg == target_regime], dtype=float)
