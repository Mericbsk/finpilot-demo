"""Bear Researcher Agent — INT-5 (TradingAgents Bull/Bear Debate)

Generates 3-5 bearish arguments for a symbol based on scanner + research data.
Runs in parallel with BullResearcherAgent inside the CEO LangGraph workflow.

Output shape per symbol::

    {
        "arguments": list[str],     # 3–5 concise bearish points
        "strength_score": float,    # 0.0–1.0  (LLM-assessed conviction)
        "key_risks": list[str],     # top 1–2 near-term risks
        "provider": str,
        "latency_ms": float,
    }
"""
...
_SYSTEM_PROMPT = (
    "Sen deneyimli bir yatırım bankası BEAR araştırma analistisin. "
    "Görevin: verilen hisse için en güçlü SATICI / DÜŞÜŞ risklerini belirlemek. "
    "Teknik ve temel göstergeleri, haberleri ve piyasa bağlamını kullanarak "
    "3-5 maddeli kısa, net, kanıta dayalı ayı argümanları yaz. "
    "Sadece zayıf yönleri, riskleri ve potansiyel katalizörleri vurgula. "
    "Yanıtını YALNIZCA JSON formatında ver:\n"
    '{"arguments": [...], "strength_score": 0.0-1.0, "key_risks": [...]}'
)


def _build_bear_prompt(symbol: str, scan: dict[str, Any], research: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Risk/Ödül: {rr} | Sosyal Sentiment: {sentiment:.0%}\n"
        f"{news_text}\n\n"
        f"{symbol} için en güçlü RİSK / AYICI argümanları üret."
    )


class BearResearcherAgent(BaseAgent):
    """Generates bear-case arguments for each symbol using the LLM router."""
    ...
    def run(self, context: AgentContext, **kwargs: object) -> AgentResult:  # noqa: D102
        ...
        for sym in context.symbols:
            ...
            prompt = _build_bear_prompt(sym, scan, research)
            try:
                ...
                results[sym] = {
                    ...
                    "key_risks": parsed.get("key_risks", [])[:2],
                    "provider": response.provider,
                    "latency_ms": response.latency_ms,
                }
                logger.info(
                    "BearResearcher: %s — %d args, strength=%.2f via %s",
                    sym,
                ...
                logger.warning("BearResearcher: %s failed: %s", sym, exc)
