...
for i in range(1, len(df_15m)):
    ...
    if (
        ...
        score += 1
    if 30 <= df_15m["rsi"].iloc[i] <= 45 and df_15m["rsi"].iloc[i] > df_15m["rsi"].iloc[i - 1]:
        score += 1
    if df_15m["macd_hist"].iloc[i] > 0 and df_15m["macd_hist"].iloc[i - 1] < 0:
        score += 1
    if df_15m["Volume"].iloc[i] >= df_15m["vol_med20"].iloc[i] * 1.2:
        score += 1
...
fig.add_trace(
    go.Scatter(
        x=df_15m.index, y=df_15m["bb_upper"], line=dict(color="blue", width=1), name="BB Üst"
    )
)
fig.add_trace(
    go.Scatter(
        x=df_15m.index, y=df_15m["bb_lower"], line=dict(color="blue", width=1), name="BB Alt"
    )
)
