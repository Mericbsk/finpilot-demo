...
| 3E.11 | **TRİYAJ TAMAMLANDI (2026-07-29):** 15 hatanın tamamı tek tek incelendi. **10 tanesi düzeltildi** (Level A, test-katmanı, production kodu değişmedi): `test_score_engine_catalyst_off_by_default` + `test_score_engine_squeeze_off_by_default` (Faz 5 / 2026-06-20 `b2bdba0` skor×0.5 düşüşü için test beklentisi bayıktı), `test_defensive_strategy` + `test_squeeze_factor_high_short_low_float` + `test_get_alpha_features_skips_squeeze_when_disabled` (yerel `.env`'deki `FINPILOT_ENABLE_ALPHA_V2=1` testlere sızıyordu, izolasyon eklendi), `test_returns_none_on_insufficient_data` (2026-07-15 `2c60744` `_unavailable_result` sözleşmesi için test bayıktı), `test_returns_expected_keys` + `test_rounds_price_to_four_decimals` (`_fetch_price_sync` Alpaca-önce mimarisine taşınmış, test eski `yfinance.Ticker` mock hedefini kullanıyordu), `test_auth_register_login_and_me_flow` (`core.config.DB_PATH` ilk import'ta donuyor, `monkeypatch.setenv` tek başına yetersiz — gerçek kalıcı `data/finpilot.db`'ye yazıp testler arası kullanıcı birikiyordu), `test_all_fields_present_and_lint_clean` (glossary `prob_band` metni "garanti/guarantee" kelimesini olumsuzlama içinde kullanıyordu, linter negation-aware değil — içerik yeniden yazıldı, linter kuralı GEVŞETİLMEDİ). **Sonuç doğrulandı:** 744 passed, 5 failed, 6 skipped (önce: 734/15/6). **5 tanesi kasıtlı olarak düzeltilmedi:** 2'si (`test_dedup_symbol_day_*`) `tests/test_full_universe_robustness.py` içinde — bu dosya git'e henüz commit edilmemiş (untracked), başka bir eş zamanlı oturumun WIP'i, dokunulmadı; 1'i (`test_server_port_in_use`) gerçek soket bind zamanlamasına dayanan, ortam-bağımlı flaky test; 2'si (`scanner_rollout/test_runtime_baseline.py`) `entry_ok` için score==2 + alignment_ratio≥0.66 durumunda geçerli olması beklenen bir "gevşetilmiş giriş kapısı" bekliyor ama `scanner/evaluate.py`'de `entry_ok = bool(score == 3)` sabit kodlanmış — bu ya hiç uygulanmamış planlı bir özellik ya da geçmişte kaldırılmış bir davranış (git log: bu test dosyası sadece bir kez, 2026-05-05'te "thanks" başlıklı toplu commit'te eklenmiş, iz sürülemedi). Canlı sinyal üretim mantığını etkilediği için **ürün/quant kararı gerekiyor**, dokunulmadı. | Level A (10/15) + Level B/C (5/15) | **Tamamlandı ve doğrulandi** (10) / **Karar bekliyor** (5) | pytest çıktısı (744 passed, 5 failed), commit `068f2aa`, bu oturum |
...
| 3E.7 | **Güncellendi (2026-07-29):** gerçek sayı doğrulandı — yerel `.env` içinde 18 `FINPILOT_ENABLE_*`/`FRED_*`/`SEC_EDGAR_*` anahtarı var (ör. `FINPILOT_ENABLE_ALPHA_V2=1`, `FINPILOT_ENABLE_SQUEEZE_FACTOR=1`, `FINPILOT_ENABLE_EDGAR_CATALYST=1`, `FINPILOT_ENABLE_LOTTERY_FADE=1`, `FINPILOT_ENABLE_FRED_MACRO=1` — hepsi kişisel araştırma/backtest için AÇIK), ama `.env.example` bunların hiçbirini belgelemiyor (sadece 9 farklı, daha eski bayrağı belgeliyor) ve `render.yaml` sadece 1 tanesini (`FINPILOT_ENABLE_DISTRIBUTION`) içeriyor. **Belirsizlik:** `.env.example` yorumları bu deneysel faktörlerin çoğu için "Default OFF; shadow modda ölç, sonra promote et" diyor — yani production'ın bunları AÇMAMASI kasıtlı/güvenli olabilir, ya da gözden kaçmış olabilir. **Bu oturumda render.yaml'a DOKUNULMADI** — hangi faktörlerin production'a hazır olduğu bir ürün/quant kararı, ben tek taraflı karar veremem; yanlış uygularsam canlı sinyal üretimini değiştirebilir. Öneri aynı kalıyor: render.yaml'ı `.env.example`'dan üreten bir script YA DA iki dosyayı karşılaştıran bir CI kontrolü + hangi deneysel faktörlerin prod'a alınacağına dair açık bir karar. | Level B | **Onay bekliyor — DOKUNULMADI (risk: canlı sinyal davranışı)** | `render.yaml` vs `.env` karşılaştırması (18 anahtar), bu oturum |
