...
try:
    ...
    assert (rsi_valid >= 0).all() and (rsi_valid <= 100).all(), f"RSI aralık dışı"
    log_pass("RSI(14)", f"Son: {rsi_val:.2f}, Min: {rsi_valid.min():.2f}, Max: {rsi_valid.max():.2f}")
    ...
    # Tüm sütunlar
    all_indicator_cols = ["ema50", "ema200", "rsi", "macd_hist", "bb_upper", "bb_lower", "atr", "vol_med20", "vol_avg10"]
...
try:
    ...
    for m in metrics:
        log_info(f"  Horizon {m['horizon']}d", f"Return: {m.get('return_pct', 'N/A')}%, Z-score: {m.get('z_score', 'N/A')}")
...
try:
    ...
    try:
        ...
        from scanner import evaluate_symbol  # bu çalışıyor
        log_warn("Risk Yönetimi", "calculate_risk_management scanner paketinde değil, scanner.py'de tanımlı — import sorunu")
...
try:
    ...
    expected_metrics = ["total_return", "win_rate", "sharpe_ratio", "max_drawdown", "profit_factor", "cagr"]
...
try:
    from altdata import get_sentiment_score, get_onchain_metric, get_latest_alt_data
...
try:
    ...
    # Normal mod
    reset_to_default()
    normal_vals = {k: get_setting(k) for k in ["vol_multiplier", "momentum_pct", "trend_gap_pct", "min_alignment_ratio", "min_signal_score", "min_price", "min_avg_vol", "momentum_z_threshold"]}
    log_pass("Normal Mod Ayarları", json.dumps(normal_vals, default=str))

    # Agresif mod
    apply_aggressive_mode()
    agg_vals = {k: get_setting(k) for k in normal_vals.keys()}
    log_pass("Agresif Mod Ayarları", json.dumps(agg_vals, default=str))

    # Farkları kontrol et
    diffs = {k: f"{normal_vals[k]} → {agg_vals[k]}" for k in normal_vals if normal_vals[k] != agg_vals[k]}
    ...
    if os.path.exists(wfo_file):
        ...
        for col in df_wfo.columns:
            if col.startswith("best_"):
                ...
                if len(vals) == 1:
                    log_warn(f"WFO {col}", f"Tek değer: {vals[0]} — tüm pencereler aynı parametre seçti (overfitting riski)")
                else:
                    log_pass(f"WFO {col}", f"Değerler: {vals}")

        # Sonuç: Neden 0 trade?
        log_info("WFO 0-Trade Analizi", "Tüm pencereler gevşek parametreleri seçiyor ama yine de trade üretemiyor. Olası nedenler: (1) Backtest giriş koşulları çok sıkı, (2) Test penceresi çok kısa (30 gün), (3) 2023 veri setinde trendsiz dönem")
except Exception as e:
...
try:
    ...
    from drl.config import FeatureSpec, RewardWeights, TransactionCostModel, PilotShieldConfig
    ...
    rw = RewardWeights()
    log_pass("Reward Weights", f"pnl={rw.pnl}, dd={rw.drawdown}, cost={rw.transaction_cost}, lev={rw.excess_leverage}, regime={rw.regime_alignment}")
    ...
    log_pass("PilotShield", f"max_pos={ps.max_position_size}, max_lev={ps.max_leverage}, risk={ps.risk_appetite}")

    # Reward ağırlık tutarlılığı — toplam çok yüksek olmamalı
    total_weight = rw.pnl + rw.drawdown + rw.transaction_cost + rw.excess_leverage + rw.regime_alignment
...
try:
    from telegram_alerts import TelegramAlerter, get_alerter_instance
...
if shortlists:
    ...
    log_pass("Shortlist Analizi", f"{total_rows} toplam satır, {len(unique_dates)} farklı gün, {len(all_symbols)} farklı hisse")
