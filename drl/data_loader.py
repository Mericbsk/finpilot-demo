...
# ---------------------------------------------------------------------------
# Sprint 18: HMM-based regime detection with rule fallback
# ---------------------------------------------------------------------------
...
try:
    ...
    _hmm_module = None  # type: ignore[assignment]


def _hmm_regime_series(prices: pd.Series, n_components: int = 3) -> pd.Series:
    """Run 3-state GaussianHMM on log-returns and map states to regime labels.

    State mapping uses the mean return of each hidden state:
    - Highest mean → "trend"   (strong positive drift)
    - Lowest mean  → "volatility" (often negative/choppy)
    - Middle       → "range"   (low-drift, mean-reverting)

    Returns a Series of str labels aligned with the input prices index.
    """
    log_ret = np.log(prices / prices.shift(1)).dropna().values.reshape(-1, 1)

    if len(log_ret) < max(50, n_components * 20):
        raise ValueError("Not enough data for HMM fitting")
    ...
    # Map states by their mean: high → trend, low → volatile, mid → range
    means = model.means_.flatten()
    sorted_indices = np.argsort(means)  # ascending
    state_map = {}
    state_map[sorted_indices[0]] = "volatility"  # lowest mean
    state_map[sorted_indices[-1]] = "trend"  # highest mean
    for idx in sorted_indices[1:-1]:
        state_map[idx] = "range"

    labels = [state_map[s] for s in states]
    # Re-align: log_ret drops first row, prepend first label to match length
    labels = [labels[0]] + labels
    return pd.Series(labels, index=prices.index, name="regime")


def _rule_based_regime(df: pd.DataFrame) -> pd.Series:
    """Fallback rule-based regime detection (original logic)."""
    conditions = [
...
def _assign_regime_labels(df: pd.DataFrame) -> pd.DataFrame:
    """Assign regime columns using HMM when available, else rules.

    Produces columns: regime, regime_trend, regime_range, regime_volatility
    """
    try:
        if _HAS_HMM and len(df) >= 50:
            df["regime"] = _hmm_regime_series(df["close"]).values
            logger.debug("Regime detection: HMM (3-state)")
        else:
            raise ValueError("HMM unavailable or too few rows")
    except Exception as e:
        logger.debug("Falling back to rule-based regime: %s", e)
        df["regime"] = _rule_based_regime(df).values
