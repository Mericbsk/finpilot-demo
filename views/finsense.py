...
def render_compound_interest_calculator():
    ...
    c3.metric("Gelecekteki Değer", f"₺{future_value:,.0f}", delta=f"%{((future_value/total_invested)-1)*100:.1f}")
    ...
    st.area_chart(df_calc.set_index("Yıl")[["Yatırılan Ana Para", "Toplam Değer"]], color=["#94a3b8", "#00e6e6"])
...
def render_quiz_module(terms):
    ...
    if q:
        ...
        for i, opt in enumerate(q["options"]):
            ...
            if cols[i % 2].button(opt["term"].split('(')[0].strip(), key=btn_key, use_container_width=True, disabled=st.session_state.quiz_state["answered"]):
                ...
                if opt["term"] == q["term"]["term"]:
                    ...
                    st.session_state.quiz_state["feedback"] = f"❌ Yanlış. Doğru cevap: **{q['term']['term']}**"
                st.rerun()
        ...
        st.markdown("---")
        st.metric("Skor", f"{st.session_state.quiz_state['score']} / {st.session_state.quiz_state['total']}")
...
def render_finsense_page():
    ...
    tab_dict, tab_quiz, tab_strat, tab_calc = st.tabs(["📚 İnteraktif Sözlük", "🧠 Quiz Modu", "🧩 Strateji Çözücü", "🧮 Hesaplayıcılar"])
    ...
    with tab_dict:
        if not terms:
            st.error("Sözlük verisi bulunamadı. Lütfen 'data/dictionary.json' dosyasını kontrol edin.")
        else:
            # Filtreleme Alanı
            col_search, col_filter = st.columns([2, 1])
            with col_search:
                search_query = st.text_input("🔍 Sözlükte Ara", placeholder="Örn: Enflasyon, Hisse, Risk...").lower()
            with col_filter:
            ...
            for category in categories:
                ...
                category_terms = [
                    ...
                    and (search_query in t["term"].lower() or search_query in t["definition"].lower())
                ]
    ...
    # Alt Bilgi
    st.markdown("---")
    st.caption("ℹ️ Bu içerikler uluslararası finansal okuryazarlık standartlarına (OECD/INFE) uygun olarak hazırlanmıştır.")
