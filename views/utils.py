...
def get_gemini_research(symbol: str, language: str = "tr") -> str:
    ...
    try:
        ...
        # İlk 12 haberi al
        news_context = "\n\n".join([f"Tarih: {r.get('date', 'Belirsiz')}\nBaşlık: {r['title']}\nKaynak: {r['source']}\nÖzet: {r['body']}" for r in unique_results[:12]])

        # 2. Gemini ile Analiz
        genai.configure(api_key=api_key)
        # Standart modele geri dönüyoruz
        ...
        if "429" in error_msg:
            return "⚠️ Kota Aşımı: Google Gemini API ücretsiz kota sınırına ulaşıldı. Lütfen yaklaşık 1 dakika bekleyip tekrar deneyin. (Model: gemini-2.0-flash)"
