...
Reads historical signals from SQLite (primary) or signal_log.csv (legacy),
...
# ---------------------------------------------------------------------------
# Signal Logging (Write) — Sprint 20: DB-first, CSV fallback
# ---------------------------------------------------------------------------
def log_signals(df: pd.DataFrame) -> int:
    ...
    # Fallback / backup: CSV
    if count == 0:
        count =  — Sprint 20: DB-first, CSV fallback
# ---------------------------------------------------------------------------
    ...
    if _SIGNAL_DB_AVAILABLE:
        try:
            ...
            if rows:
                ...
                # Normalise column names to match legacy expectation
...
def _write_signals_csv(signals: list[dict]) -> int:
    ...
    try:
        with open(SIGNAL_LOG_PATH, "a", newline="", encoding="utf-8") as f:
            ...
            for s in signals:
                writer.writerow([
                    s["timestamp"], s["symbol"], s.get("price", ""),
                    s.get("stop_loss", ""), s.get("take_profit", ""),
                    s.get("score", ""), s.get("strength", ""),
                    s.get("regime", ""), s.get("sentiment", ""),
                    s.get("onchain", ""), s.get("entry_ok", ""),
