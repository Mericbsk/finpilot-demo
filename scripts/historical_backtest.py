...
def historical_backtest(
    symbols: list, start_date: datetime, end_date: datetime, initial_capital: float = 10000
):
    """
    ...
    if all(results.values()):
        ...
        print(
            f"   En İyi Drawdown:  {best_drawdown[0]} ({best_drawdown[1]['max_drawdown_pct']:.2f}%)"
        )
    ...
    for name, engine in engines.items():
        ...
        if closed_trades:
            ...
            for trade in closed_trades[-5:]:  # Son 5 trade
                print(
                    f"   {trade['symbol']:5s} {trade['profit']:+7.2f} ({trade['profit_pct']:+.1f}%) - {trade['hold_days']} days"
                )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--symbols", type=str, default="AAPL,MSFT,GOOGL", help="Semboller (virgülle ayrılmış)"
    )
    parser.add_argument(
        "--capital", type=float, default=10000, help="Başlangıç sermayesi (default: 10000)"
    )
    ...
    results, engines = historical_backtest(
        symbols=symbols, start_date=start_date, end_date=end_date, initial_capital=args.capital
    )
