...
def get_market_regime_status(symbols):
    ...
    try:
        ...
        if close_val < ema_val:
            return {'safe': False, 'reason': f"Düşüş Trendi (Fiyat < EMA50). {index_symbol} @ {close_val:.2f} < {ema_val:.2f}"}
            
        # 2. Red Day Filter
        if close_val < open_val:
             return {'safe': False, 'reason': f"Kırmızı Gün (Close < Open). {index_symbol} bugün satıcılı."}
...
def main():
    ...
    parser.add_argument("--aggressive", action="store_true", help="Eşikleri gevşeterek daha fazla fırsat yakala")
