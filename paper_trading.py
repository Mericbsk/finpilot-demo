...
from scanner.signals import safe_float, analyze_price_momentum, check_volume_spike
from stable_baselines3 import PPO
from drl.config import DEFAULT_CONFIG
from drl.feature_pipeline import FeaturePipeline, FeatureFrame
...
class PaperTradingEngine:
    ...
    def generate_signal(self, symbol: str, df: pd.DataFrame) -> Dict:
        ...
        if close > ema_20 > ema_50: score += 2
        elif close > ema_20: score += 1
        elif close < ema_20 < ema_50: score -= 2
        elif close < ema_20: score -= 1
        
        if momentum.get('momentum_bias') == 'BULLISH': score += 1
        elif momentum.get('momentum_bias') == 'BEARISH': score -= 1
        if volume_spike: score += 1
        
        if score >= 2: scanner_action = "BUY"
        elif score <= -2: scanner_action = "SELL"
        else: scanner_action = "HOLD"
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        total_return = (df['total_value'].iloc[-1] - self.initial_capital) / self.initial_capital * 100
...
def run_paper_trading_day(engines: Dict[str, PaperTradingEngine], 
                          symbols: List[str], 
                          test_date: datetime) -> Dict:
